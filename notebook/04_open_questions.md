...
## From E00 LM-vs-denoiser side experiment (W18 2026)

- **Q19. Does the impairment hierarchy from the sinusoid-scale LM-vs-denoiser sweep carry over to OFDM-grid scale?** The E00 side experiment confirmed `iid < ar1_coloured < wiener_phase < dc_offset` for how much LM-style next-sample prediction degrades vs supervised denoising. Before committing to E07's masked-RE recipe, repeat the comparison at LTE-5 EPA scale: train one masked-RE model and one supervised twin, then introduce the same impairment families (CFO, IQ imbalance, PA non-linearity, phase noise) and check whether the gap matches the sinusoid-scale ranking. If yes → E07 needs an auxiliary supervised signal at impairment surfaces it cannot see through. If no → masked-RE may be more robust than the sinusoid-scale prediction suggests, and we can pursue the SSL-only recipe with more confidence. See [discussion 009](01_discussions/009_lm_vs_denoiser_equivalence.md).
- **Q20. What is the SSL-vs-supervised gap as a function of impairment magnitude?** The W18 sweep used a single magnitude per mode (`α_AR1 = 0.9`, `σ_μ = 0.5`, etc.). The shape of the equivalence boundary — at what magnitude the gap opens, how it scales — would tell us whether the cognitive receiver should switch training recipes online (e.g., supervised when impairments are detected, SSL otherwise) or stay on a single recipe with a known operating envelope.
